...
?>
   ...
   /*  if(!@$_POST['submit'])
     ...
     // $userid=$_SESSION['uid'];
      ...
      if($des)
      {
     $myquery ="SELECT * from `request` where doctid='$doctid' and rid='$des'";
...
?>
    ...
    //  session_start();
     $doctid=$_SESSION['sid'];
     $myquery ="SELECT * from `request` where doctid='$doctid' and emrstatus='pending'";
     $result = mysqli_query($connect,$myquery);
     if($result)
     {
     $num_rows=mysqli_num_rows($result);
...
//session_start();
include ('connect.php');
$query ="SELECT * from `udapp` where udname='$sname'";
...
while($row = mysqli_fetch_array($result))
{
        ...
        echo $row['upname'],       "having schedule on"      ,$row['fdate'],     $row['ftime'];
       echo "<br>";
